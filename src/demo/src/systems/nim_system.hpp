...
namespace systems {
	...
	class Nim_comp : public ecs::Component<Nim_comp> {
		public:
			...
			Nim_comp() = default;
			Nim_comp(ecs::Entity_manager& manager, ecs::Entity_handle owner)
	...
	class Nim_sequence {
		public:
			...
			Nim_sequence(std::vector<util::Str_id> affected_entities)
			    : _affected_entities(std::move(affected_entities))
			    , _entity_states(_affected_entities.size()) {
			}

			auto affected_entities()const noexcept -> auto& {
				return _affected_entities;
			}

			auto frame_length(int frame)const noexcept {
				return _frame_lengths[frame % _frame_lengths.size()];
			}

			auto frames()const noexcept {
				return _frame_lengths.size();
			}

...
namespace mirrage {
	                   ...
	                   bool                            interactive_all)
	{
		...
		if(ansi) {
			...
			model_progress.set_bar_width(60);
		...
		importer.SetPropertyFloat(AI_CONFIG_GLOBAL_SCALE_FACTOR_KEY, cfg.scale);
		...
		importer.SetPropertyFloat(AI_CONFIG_PP_GSN_MAX_SMOOTHING_ANGLE, cfg.smooth_normal_angle);
		importer.SetProgressHandler(new MyProgressHandler(ansi ? &model_progress : nullptr));

		auto flags = aiProcess_JoinIdenticalVertices | aiProcess_GenSmoothNormals | aiProcess_Triangulate
		             | aiProcess_ImproveCacheLocality | aiProcess_RemoveRedundantMaterials
		             | aiProcess_OptimizeMeshes | aiProcess_ValidateDataStructure | aiProcess_FindDegenerates
		             ...
		             | aiProcess_TransformUVCoords | aiProcess_SortByPType | aiProcess_GlobalScale;

		if(cfg.discard_normals)
		...
		// load skeleton
		auto skeleton = parse_skeleton(model_name, output, *scene, cfg, fit_skeleton, guess_scale);
		auto rigged   = !skeleton.bones.empty();
		parse_animations(model_name, name, output, *scene, cfg, skeleton);
		...
		if(index_count > 0 && !cfg.only_animations) {
			// load materials
			...
			for(auto& mat : materials) {
				...
				auto mat_id = material_names.size() > (processed_materials - 1)
				                      ? material_names[processed_materials - 1]
				...
				if(ansi) {
					model_progress.set_progress(
					        30.f + 30.f * (processed_materials / static_cast<float>(materials.size())));
				}
			}
			...
			auto traverse = [&](const aiNode* node, auto parent_transform, auto&& recurse) -> void {
				...
				for(auto& mesh_idx : gsl::span(node->mMeshes, node->mNumMeshes)) {
					...
					for(auto i = std::size_t(0); i < mesh->mNumVertices; i++) {
						...
						auto uv = mesh->mTextureCoords[0] ? glm::vec2(mesh->mTextureCoords[0][i].x,
					...
					if(rigged && mesh->HasBones()) {
						...
						for(auto& bone : gsl::span(mesh->mBones, mesh->mNumBones)) {
							auto bone_idx_mb = util::find_maybe(skeleton.bones_by_assimp_bone, bone);
							if(bone_idx_mb.is_nothing())
								bone_idx_mb = util::find_maybe(skeleton.bones_by_name, bone->mName.C_Str());
							auto bone_idx = bone_idx_mb.get_or_throw("References bone doesn't exist: ",
							                                         bone->mName.C_Str());
			...
			// write file
			auto model_out_filename = output + "/models/" + name.get_or(util::to_lower(model_name)) + ".mmf";
			auto model_out_file =
			...
			if(rigged)
				header.vertex_size = gsl::narrow<std::uint32_t>(rigged_vertices.size()
				                                                * sizeof(decltype(rigged_vertices[0])));
			else
				header.vertex_size = gsl::narrow<std::uint32_t>(normal_vertices.size()
				                                                * sizeof(decltype(normal_vertices[0])));
			...
			for(auto& sub_mesh : sub_meshes) {
				...
				write(model_out_file, sub_mesh.material_id);
			}
			...
			model_progress.set_postfix_text("No model to convert");
			model_progress.set_progress(100.0001f);
		}

...
namespace mirrage {
	...
	namespace {
		...
		constexpr unsigned char type_tag[] = {0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A};
		...
		void generate_mip_maps(Image_data<T>& image) {
			for(std::uint32_t i=1u; i<image.mip_levels.size(); i++) {
				...
				for(std::uint32_t y=0; y<height; y++) {
					for(std::uint32_t x=0; x<width; x++) {
						auto v = image.pixel(i-1, x*2,   y*2  )
		...
		void store_texture(Image_data<float>& image, const std::string& ouput,
		                   bool srgb = true) {
			if(srgb) {
			...
			for(std::uint32_t i=0; i<rgb8_image.mip_levels.size(); i++) {
				...
				out.write(reinterpret_cast<char*>(rgb8_image.mip_levels[i].data()),
				          size);

...
namespace input {
	...
	namespace {
		...
		std::tuple<std::unordered_map<Context_id, Context>, Context_id>
		  load_context_map(asset::Asset_manager& assets,
		                   std::unordered_map<Context_id, Context>& map) {
	...
	Input_mapper::Input_mapper(util::Message_bus& bus,
	...
	namespace {
		...
		void process_release(util::Message_bus& bus, const Reaction& action,
			...
			switch(action.type) {
				...
				case Reaction_type::once:
					bus.send<Once_action>(action.action, src);
					break;
				default:
					break;
			}
	...
	void Input_mapper::on_mouse_wheel_change(glm::vec2 rel) {
		...
		switch(action.type) {
			...
			case Reaction_type::once:
				_bus.send<Once_action>(action.action, Input_source(0));
				break;
	...
	void Input_mapper::on_pad_button_changed (Input_source src, Pad_button b,
	                                          float intensity) {
		find_maybe(_active_context->pad_buttons, b).process([&](auto& action) {
	...
	Input_mapping_updater::Input_mapping_updater(asset::Asset_manager& assets)
	    : _assets(assets) {
	...
	void Input_mapping_updater::save() {
		save_context_map(_assets, _context, _default_context_id);
	}

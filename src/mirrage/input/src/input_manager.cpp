...
namespace input {
	...
	Sdl_event_filter::Sdl_event_filter(Input_manager& e) :_manager(&e) {
	...
	void Input_manager::add_event_filter(Sdl_event_filter& f) {
		_event_filter.push_back(&f);
	}
	void Input_manager::remove_event_filter(Sdl_event_filter& f) {
	...
	void Input_manager::capture_mouse(bool enable) {
		if(_mouse_captured != enable) {
			...
			if(!enable && _sdl_window) {
				SDL_WarpMouseInWindow(_sdl_window, _mouse_capture_screen_pos.x,
				                      _mouse_capture_screen_pos.y);
			}

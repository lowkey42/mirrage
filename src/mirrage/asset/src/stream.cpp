...
namespace mirrage::asset {
	...
	class stream::fbuf : public std::streambuf {
		...
		pos_type seekoff(std::streamoff pos, std::ios_base::seekdir dir, std::ios_base::openmode mode) {
			switch(dir) {
				...
				default: PHYSFS_seek(file, PHYSFS_fileLength(file) + pos); break;
			}
	...
	size_t stream::length() const noexcept { return PHYSFS_fileLength((PHYSFS_File*) _file); }
	...
	ostream::ostream(AID aid, Asset_manager& manager, const std::string& path)
	  : stream(aid, manager, (File_handle*) PHYSFS_openWrite(path.c_str()), path)

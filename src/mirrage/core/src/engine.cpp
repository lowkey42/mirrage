...
namespace {
	...
	struct Engine_event_filter : public input::Sdl_event_filter {
		...
		Engine_event_filter(Engine& engine)
		    : Sdl_event_filter(engine.input())
		    , _engine(engine) {
		}
	...
	Engine::Engine(const std::string& title, std::uint32_t version_major, std::uint32_t version_minor,
	               bool debug, int argc, char** argv, char**)
	  : _screens(*this),
	    ...
	    _sdl(),
	    _graphics_context(std::make_unique<graphic::Context>(title, graphic::make_version_number(version_major,version_minor,0),
	                                                         "No Engine", graphic::make_version_number(0,0,0),
	                                                         debug, *_asset_manager)),

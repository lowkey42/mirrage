	...
	class purgatory {
		...
		public:
			...
			auto add(T&& obj) -> T& {
				...
				static_assert(!std::is_lvalue_reference<T>::value, "The object has to be passed as an rvalue reference to transfer its ownership!");

				auto ptr   = _reserve(sizeof(T), alignof(T));
				auto grave = new(ptr) T(std::move(obj));

				_entries.emplace_back(+[](void* ptr) {

...
namespace util {
	...
	template<typename T>
	inline glm::tvec2<T, glm::highp> clamp(glm::tvec2<T, glm::highp> v, glm::tvec2<T, glm::highp> min, glm::tvec2<T, glm::highp> max)noexcept {
		return glm::tvec2<T, glm::highp>(clamp(v.x, min.x, max.x), clamp(v.y, min.y, max.y));
	}
	template<typename T>
	inline glm::tvec3<T, glm::highp> clamp(glm::tvec3<T, glm::highp> v, glm::tvec3<T, glm::highp> min, glm::tvec3<T, glm::highp> max)noexcept {
		return glm::tvec3<T, glm::highp>(clamp(v.x, min.x, max.x), clamp(v.y, min.y, max.y), clamp(v.z, min.z, max.z));
	}

	template<typename T>
	inline auto rotate(glm::tvec2<T, glm::highp> v, Angle a)noexcept
			-> glm::tvec2<T, glm::highp> {
		auto r = glm::rotate(remove_units(v), a.value());
		return {T{r.x}, T{r.y}};
	}
	template<typename T>
	inline auto rotate(glm::tvec3<T, glm::highp> v,

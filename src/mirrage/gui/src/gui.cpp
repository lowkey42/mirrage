...
namespace gui {
	...
	namespace {
		...
		class Gui_event_filter : public input::Sdl_event_filter {
			public:
				Gui_event_filter(input::Input_manager& input_mgr, nk_context* ctx,
				                 glm::vec4 viewport, const glm::mat4& ui_matrix)
				...
				bool propagate(SDL_Event& evt) override {
					switch(evt.type) {
						...
						case SDL_MOUSEBUTTONUP:
							sdl_to_nk_mb(evt.button.button).process([&](auto button) {
								auto p = glm::unProject(glm::vec3{evt.button.x, evt.button.y, 0.99f},
								                        glm::mat4(1), _ui_matrix, _viewport);
						...
						case SDL_MOUSEWHEEL:
							...
							nk_input_scroll(_ctx, nk_vec2(static_cast<float>(evt.wheel.x),
							                              static_cast<float>(evt.wheel.y)));
							return false;
		...
		auto get_buffer_data_ref(nk_buffer& src) -> gsl::span<const T> {
			...
			if(size % 4 != 0) {
				...
				INVARIANT(align_diff+info.allocated <= info.size, "Can't align buffer: "
	...
	namespace detail {
		struct Nk_renderer {
			...
			Nk_renderer(Gui_renderer& renderer)
			    : renderer(renderer) {
			}

			void draw(nk_context& ctx, glm::vec4 viewport, glm::vec2 screen_size,
	...
	namespace {
		...
		auto build_ui_mat(glm::vec2 size) {
			auto m = glm::ortho(-size.x/2.f,  size.x/2.f,
			                     size.y/2.f, -size.y/2.f,
			                           -1.f,         1.f);
			m[1][1] *= -1.f;
	...
	Gui::Gui(glm::vec4 viewport, asset::Asset_manager& assets,
	         input::Input_manager& input, Gui_renderer& renderer)
	    : _renderer(renderer)
	    , _impl(std::make_unique<PImpl>(viewport, assets, input, renderer)) {
	...
	void Gui::draw() {
		_impl->renderer.draw(_impl->ctx.ctx,
		                     _impl->viewport, _impl->screen_size, _impl->ui_matrix);
	}
	...
	auto Gui::centered_right(int width, int height) -> struct nk_rect {
		return nk_rect(_impl->screen_size.x - width,
		               _impl->screen_size.y/2.f - height/2.f,
		               width,
		               height);
	}

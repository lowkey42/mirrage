...
namespace mirrage::renderer {
	...
	void load_component(ecs::Deserializer& state, Directional_light_comp& comp)
	{
		...
		state.read_virtual(sf2::vmember("source_radius", src_radius),
		                   ...
		                   sf2::vmember("light_particles", comp._light_particles),
	...
	auto Directional_light_comp::calc_shadowmap_view_proj(glm::vec3 position, glm::quat orientation) const
	        -> glm::mat4
	{
		auto inv_view = glm::toMat4(orientation);
		inv_view[3]   = glm::vec4(position, 1.f);

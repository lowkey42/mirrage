...
namespace renderer {
	...
	namespace {
		auto build_render_pass(Deferred_renderer& renderer,
			...
			pipeline.add_push_constant("camera"_strid, sizeof(glm::mat4),
			                           vk::ShaderStageFlagBits::eVertex);

			pipeline.vertex<gui::Gui_vertex>(0, false,
			                                 0, &gui::Gui_vertex::position,
			                                 1, &gui::Gui_vertex::uv,
			                                 2, &gui::Gui_vertex::color);

			auto& pass = builder.add_subpass(pipeline)
			                    .color_attachment(screen, graphic::all_color_components,
	...
	Gui_pass::Gui_pass(Deferred_renderer& drenderer,
	                   ecs::Entity_manager&,
	                   util::maybe<Meta_system&>)
	    : _renderer(drenderer)
	    , _sampler(drenderer.device().create_sampler(1, vk::SamplerAddressMode::eClampToEdge,
	    ...
	    , _mesh_buffer(drenderer.device(), max_render_buffer_size,
	...
	Gui_pass::Loaded_texture::Loaded_texture(int handle,
		...
		auto desc_image = vk::DescriptorImageInfo{sampler, this->texture->view(),
		                                         vk::ImageLayout::eShaderReadOnlyOptimal};

		auto desc_write = vk::WriteDescriptorSet{*descriptor_set, 0, 0,
		                                         1,
	...
	auto Gui_pass::load_texture(int width, int height, int channels,
	                            const std::uint8_t* data) -> std::shared_ptr<struct nk_image> {
		auto handle = _next_texture_handle++;

		auto dimensions = graphic::Image_dimensions_t<graphic::Image_type::single_2d>{
				gsl::narrow<std::uint32_t>(width),
				gsl::narrow<std::uint32_t>(height)
		};

		auto texture = std::make_shared<graphic::Texture_2D>(_renderer.device(),
		                                                     dimensions,
		                                                     false, gsl::narrow<std::uint32_t>(channels),
		                                                     true,
		                                                     gsl::span<const std::uint8_t>{data, width*height*channels},
		                                                     _renderer.queue_family() );

		auto entry = std::make_shared<Loaded_texture>(handle, std::move(texture),
		                                              *_sampler, _renderer, *_descriptor_set_layout);
	...
	auto Gui_pass::load_texture(const asset::AID& aid) -> std::shared_ptr<struct nk_image> {
		...
		auto entry = std::make_shared<Loaded_texture>(handle, _texture_cache.load(aid),
		                                              *_sampler, _renderer, *_descriptor_set_layout);
	...
	void Gui_pass::prepare_draw(gsl::span<const std::uint16_t> indices,
		...
		auto& cb = _current_command_buffer.get_or_throw("Gui_pass::prepare_draw ha to be "
		                                                "called inside a draw call!");
		auto& fb = _current_framebuffer.get_or_throw("Gui_pass::prepare_draw ha to be "
		                                             "called inside a draw call!");
	...
	void Gui_pass::draw_elements(int texture_handle, glm::vec4 clip_rect,
		...
		cb.setScissor(0, {
		    vk::Rect2D{vk::Offset2D(clip_rect.x, clip_rect.y),
		               vk::Extent2D(clip_rect.z, clip_rect.w)}
		});

		cb.drawIndexed(count, 1, offset, 0, 0);
	}

	void Gui_pass::finalize_draw() {
		_render_pass.unsafe_end_renderpass();
	}
	...
	auto Gui_pass_factory::rank_device(vk::PhysicalDevice, util::maybe<std::uint32_t> graphics_queue,

...
namespace renderer {
	...
	namespace {
		auto build_render_pass(Deferred_renderer& renderer,
			...
			pipeline.add_push_constant("settings"_strid, sizeof(glm::vec4),
	...
	void Blit_pass::draw(vk::CommandBuffer& command_buffer,
		...
		_render_pass.execute(command_buffer, _framebuffers.at(swapchain_image), [&] {
			...
			_render_pass.push_constant("settings"_strid, glm::vec4(_tone_mapping_enabled?1:0,
			                                                       _bloom_enabled?50:0, 0, 0));
	...
	auto Blit_pass_factory::rank_device(vk::PhysicalDevice, util::maybe<std::uint32_t> graphics_queue,

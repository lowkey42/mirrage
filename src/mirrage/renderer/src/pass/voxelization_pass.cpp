...
namespace renderer {
	...
	namespace {
		...
		auto build_render_pass(Deferred_renderer& renderer,
			...
			auto data_attachment = builder.add_attachment(vk::AttachmentDescription{
			...
			pipeline.vertex<Model_vertex>(0, false,
			...
			auto& pass = builder.add_subpass(pipeline)
			                    .color_attachment(data_attachment);
			...
			out_framebuffer = builder.build_framebuffer({data.view(0), util::Rgba{0,0,0,0}},
			                                            data.width(),
			                                            data.height(),
			                                            data.layers());
		...
		auto get_voxel_format(graphic::Device& device) {
			auto format = device.get_supported_format({vk::Format::eR32G32B32A32Uint},
	...
	Voxelization_pass::Voxelization_pass(Deferred_renderer& renderer, ecs::Entity_manager& entities)
		...
		INVARIANT(renderer.gbuffer().voxels.is_nothing(), "More than one voxelization implementation active!");
	...
	void Voxelization_pass::draw(vk::CommandBuffer& command_buffer,
		...
		_render_pass.execute(command_buffer, _framebuffer, [&] {
			auto descriptor_sets = std::array<vk::DescriptorSet, 1> {
				global_uniform_set
			};
	...
	auto Voxelization_pass_factory::rank_device(vk::PhysicalDevice, util::maybe<std::uint32_t> graphics_queue,

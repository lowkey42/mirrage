...
namespace renderer {
	...
	namespace {
		...
		auto build_render_pass(Deferred_renderer& renderer,
			...
			auto shadowmap = builder.add_attachment(vk::AttachmentDescription{
				vk::AttachmentDescriptionFlags{},
				shadowmap_format,
				vk::SampleCountFlagBits::e1,
				vk::AttachmentLoadOp::eDontCare,
			...
			model_pipeline.add_descriptor_set_layout(renderer.model_loader().material_descriptor_set_layout());
			model_pipeline.vertex<Model_vertex>(0, false,
			...
			builder.add_dependency(util::nothing, vk::PipelineStageFlagBits::eColorAttachmentOutput,
			                       vk::AccessFlags{},
			                       model_pass, vk::PipelineStageFlagBits::eColorAttachmentOutput,
			                       vk::AccessFlagBits::eColorAttachmentRead|vk::AccessFlagBits::eColorAttachmentWrite);
			
			builder.add_dependency(model_pass, vk::PipelineStageFlagBits::eColorAttachmentOutput,
			                       vk::AccessFlagBits::eColorAttachmentRead|vk::AccessFlagBits::eColorAttachmentWrite,
			...
			for(auto& sm : shadowmaps) {
				auto sm_attachments = std::array<Framebuffer_attachment_desc, 2>{{
					...
					{sm.texture.view(0), util::Rgba(1.f)}
				}};
		...
		auto get_shadowmap_format(graphic::Device& device) {
			auto format = device.get_supported_format({vk::Format::eR32Sfloat, vk::Format::eR16Sfloat,
	...
	Shadowmap::Shadowmap(graphic::Device& device, std::uint32_t size, vk::Format format)
	    : texture(device, {size, size}, 1, format,
	...
	Shadowmapping_pass::Shadowmapping_pass(Deferred_renderer& renderer,
	    ...
	    , _depth(renderer.device(),
	             {gsl::narrow<std::uint32_t>(renderer.settings().shadowmap_resolution),
	              gsl::narrow<std::uint32_t>(renderer.settings().shadowmap_resolution)}, 1,
	             get_depth_format(renderer.device()),
	             vk::ImageUsageFlagBits::eDepthStencilAttachment, vk::ImageAspectFlagBits::eDepth)
	    , _shadowmap_sampler(renderer.device().create_sampler(1, vk::SamplerAddressMode::eClampToBorder,
	                                                          vk::BorderColor::eFloatOpaqueWhite,
	                                                          vk::Filter::eLinear,
	                                                          vk::SamplerMipmapMode::eNearest,
	                                                          false,
	                                                          vk::CompareOp::eLess))
	    , _shadowmap_depth_sampler(renderer.device().create_sampler(1, vk::SamplerAddressMode::eClampToBorder,
	                                                          vk::BorderColor::eFloatOpaqueWhite,
	                                                          vk::Filter::eLinear,
	                                                          vk::SamplerMipmapMode::eNearest,
	                                                          false))
		...
		INVARIANT(!renderer.gbuffer().shadowmaps_layout, "More than one shadowmapping implementation active!");
		renderer.gbuffer().shadowmaps_layout = renderer.device().create_descriptor_set_layout(shadowmap_bindings);
		...
		for(auto& sm : _shadowmaps) {
			shadowmap_infos.emplace_back(*_shadowmap_sampler, sm.texture.view(),
			                             vk::ImageLayout::eShaderReadOnlyOptimal);
		}

		auto desc_writes = std::array<vk::WriteDescriptorSet, 3>();
		desc_writes[0] = vk::WriteDescriptorSet {
			*renderer.gbuffer().shadowmaps,
			0, 0, gsl::narrow<std::uint32_t>(shadowmap_infos.size()),
		...
		renderer.device().vk_device()->updateDescriptorSets(desc_writes.size(), desc_writes.data(),
		                                                    0, nullptr);
	}

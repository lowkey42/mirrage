...
namespace mirrage::renderer {
	...
	Tone_mapping_pass::Tone_mapping_pass(Deferred_renderer& renderer,
	  ...
	  , _calc_luminance_desc_set(
	            _descriptor_set_layout.create_set(renderer.descriptor_pool(), {src.view(), src.view()}))
	  ...
	  , _adapt_luminance_renderpass(build_adapt_render_pass(renderer,
	                                                        *_descriptor_set_layout,
	                                                        _luminance_format,
	                                                        _curr_avg_luminance,
	                             ...
	                             std::size_t)
	{
		...
		_calc_luminance_renderpass.execute(command_buffer, _calc_luminance_framebuffer, [&] {
			auto descriptor_sets =
			        std::array<vk::DescriptorSet, 2>{global_uniform_set, *_calc_luminance_desc_set};
		...
		graphic::generate_mipmaps(command_buffer,
		                          ...
		                          vk::ImageLayout::eTransferSrcOptimal,
		                          vk::ImageLayout::eShaderReadOnlyOptimal,
		                          _luminance_buffer.width(),
		                          _luminance_buffer.height(),
		                          0);

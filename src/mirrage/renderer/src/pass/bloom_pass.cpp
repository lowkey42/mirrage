...
namespace renderer {
	...
	namespace {
		...
		auto build_blur_render_pass(Deferred_renderer& renderer,
			...
			for(auto i=0u; i<MipLevels; i++) {
				out_framebuffer_horizontal[i] = builder.build_framebuffer({target_horizontal.view(i), util::Rgba{}},
				                                                          target_horizontal.width(i),
				                                                          target_horizontal.height(i));

				out_framebuffer_vertical[i] = builder.build_framebuffer({target_vertical.view(i), util::Rgba{}},
	...
	Bloom_pass::Bloom_pass(Deferred_renderer& renderer,
	    ...
	    , _filter_renderpass(build_filter_render_pass(renderer, *_descriptor_set_layout,
	                                                  _bloom_buffer,
	                                                  _filter_framebuffer))
	    , _filter_descriptor_set(_descriptor_set_layout.create_set(
	                          renderer.descriptor_pool(),
	                          {src.view(0),
	                           renderer.gbuffer().avg_log_luminance
	                                   .get_or_throw("Tonemapping pass is required for bloom!")
	                                   .view()}))
	    ...
	    , _blur_buffer(renderer.device(),
	                   ...
	                   | vk::ImageUsageFlagBits::eSampled,
	                   vk::ImageAspectFlagBits::eColor)
	    , _downsampled_blur_view(renderer.device().create_image_view(_blur_buffer.image(),
	                                                                 renderer.gbuffer().color_format,
	                                                                 1,
	                                                                 blur_mip_levels-1))
	    , _blur_renderpass(build_blur_render_pass<blur_mip_levels-blur_start_mip_level>(
	                           renderer, *_descriptor_set_layout,
	                           _blur_buffer, _bloom_buffer,
	                           _blur_framebuffer_horizontal,
	                           _blur_framebuffer_vertical))

	    , _blur_descriptor_set_horizontal(_descriptor_set_layout.create_set(
	                          renderer.descriptor_pool(),
	                          {_bloom_buffer.view(), src.view(0)}))
	    ...
	    , _blur_descriptor_set_vertical_final(_descriptor_set_layout.create_set(
	                          renderer.descriptor_pool(),
	                          {_blur_buffer.view(), *_downsampled_blur_view})) {
	...
	void Bloom_pass::draw(vk::CommandBuffer& command_buffer,
		...
		// generate mip chain
		graphic::generate_mipmaps(command_buffer, _bloom_buffer.image(),
		                          vk::ImageLayout::eTransferSrcOptimal,
		                          vk::ImageLayout::eShaderReadOnlyOptimal,
		                          _bloom_buffer.width(), _bloom_buffer.height(), _bloom_buffer.mip_levels());
	...
	auto Bloom_pass_factory::rank_device(vk::PhysicalDevice, util::maybe<std::uint32_t> graphics_queue,

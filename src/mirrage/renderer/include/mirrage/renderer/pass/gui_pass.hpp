...
namespace renderer {
	...
	class Gui_pass : public Pass, public gui::Gui_renderer {
		public:
			...
			auto load_texture(int width, int height, int channels,
			                  const std::uint8_t* data) -> std::shared_ptr<struct nk_image> override;
		...
		protected:
			...
			void draw_elements(int texture_handle, glm::vec4 clip_rect,
			                   std::uint32_t offset, std::uint32_t count) override;
		...
		private:
			struct Loaded_texture {
				...
				Loaded_texture(int handle, graphic::Texture_ptr texture,
				               vk::Sampler, Deferred_renderer&, vk::DescriptorSetLayout);
			};

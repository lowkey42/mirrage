...
namespace graphic {
	...
	namespace {
		...
		class Buddy_block_alloc {
			public:
				...
				~Buddy_block_alloc() {
					INVARIANT(_allocation_count==0, "Unfree memory: "
					          <<_allocation_count<<" allocations left!");
				}
				
				auto alloc(std::uint32_t size, std::uint32_t alignment
			...
			private:
				...
				auto _index_to_offset(std::uint32_t layer,
				                      std::uint32_t idx) -> vk::DeviceSize;
	...
	class Device_memory_pool {
		public:
			...
			auto shrink_to_fit() -> std::size_t {
				auto new_end = std::remove_if(_blocks.begin(), _blocks.end(),
				                              [](auto& block) {
					return block->empty();
				});
				auto deleted = std::distance(new_end, _blocks.end());
				if(deleted>0) {
					DEBUG("Freed "<<deleted<<"/"<<_blocks.size()<<" blocks in allocator for type "<<_type);
				}
	...
	class Device_heap {
		public:
			...
			auto alloc(std::uint32_t size, std::uint32_t alignment,
				...
				auto memory = [&] {
					switch(lifetime) {
						case Memory_lifetime::temporary:
							return _temporary_pool.alloc(size, alignment);
						case Memory_lifetime::normal:
							return _normal_pool.alloc(size, alignment);
			...
			auto shrink_to_fit() -> std::size_t {
				return _temporary_pool.shrink_to_fit()
				      + _normal_pool.shrink_to_fit()
			...
			auto usage_statistic()const -> Device_memory_type_usage {
				...
				return {
					temporary_usage.reserved+normal_usage.reserved+persistent_usage.reserved,
					temporary_usage.used+normal_usage.used+persistent_usage.used,
					temporary_usage, normal_usage, persistent_usage
				};
			}
	...
	Device_memory::Device_memory(void* owner, Deleter* deleter,
	                             std::uint32_t index, std::uint32_t layer, 
	                             vk::DeviceMemory m, vk::DeviceSize o)
	    : _owner(owner), _deleter(deleter), _index(index), _layer(layer)
	    , _memory(m), _offset(o) {
	}
	

	Device_memory_allocator::Device_memory_allocator(const vk::Device& device, vk::PhysicalDevice gpu,
	                                                 bool dedicated_alloc_supported)
	    : _device(device), _is_unified_memory_architecture(true)
		...
		const auto host_visible_flags = vk::MemoryPropertyFlagBits::eHostVisible
	...
	auto Device_memory_allocator::alloc(std::size_t size, std::size_t alignment, std::uint32_t type_mask,
	                                    bool host_visible, Memory_lifetime lifetime) -> util::maybe<Device_memory> {
		...
		FAIL("No pool on the device matches the requirements. Type mask="<<type_mask
		     <<", host visible="<<host_visible);
	}
	
	auto Device_memory_allocator::alloc_dedicated(vk::Image image,
	                                              bool host_visible) -> util::maybe<Device_memory> {
		return alloc_dedicated({}, image, host_visible);
	}
	
	auto Device_memory_allocator::alloc_dedicated(vk::Buffer buffer,
	                                              bool host_visible) -> util::maybe<Device_memory> {
	...
	auto Device_memory_allocator::alloc_dedicated(vk::Buffer buffer,
		...
		FAIL("No pool on the device matches the requirements. Type mask="
...
// POOL
	...
	template<std::uint32_t MinSize, std::uint32_t MaxSize>
	auto Device_memory_pool<MinSize,MaxSize>::alloc(
	            std::uint32_t size,
	            std::uint32_t alignment) -> util::maybe<Device_memory> {
		...
		_blocks.emplace_back(std::make_unique<Buddy_block_alloc<MinSize, MaxSize>>(
				_device, _type
		));
		
		auto m = _blocks.back()->alloc(size, alignment);
		INVARIANT(m.is_some(), "Couldn't allocate "<<size<<" byte with allignment"
		          << alignment<<" from newly created block of size "<<MaxSize);
	...
	namespace {
		...
		template<std::uint32_t MinSize, std::uint32_t MaxSize>
		auto Buddy_block_alloc<MinSize,MaxSize>::alloc(
		            std::uint32_t size, std::uint32_t alignment) -> util::maybe<Device_memory> {
			...
			auto layer = gsl::narrow<std::uint32_t>(std::ceil(std::log2(static_cast<float>(
					size
			))) - log2(MinSize));
			...
			return Device_memory{this, +[](void* self, std::uint32_t i,
			                               std::uint32_t l, vk::DeviceMemory) {
				static_cast<Buddy_block_alloc*>(self)->free(l, i);
			}, index, layer, *_memory, offset};
		}
	
		template<std::uint32_t MinSize, std::uint32_t MaxSize>
		void Buddy_block_alloc<MinSize,MaxSize>::free(std::uint32_t layer,
		                                              std::uint32_t index) {
			_allocation_count--;
		...
		template<std::uint32_t MinSize, std::uint32_t MaxSize>
		auto Buddy_block_alloc<MinSize,MaxSize>::_index_to_offset(std::uint32_t layer,
		                                                          std::uint32_t idx
		                                                          ) -> vk::DeviceSize {
		...
		template<std::uint32_t MinSize, std::uint32_t MaxSize>
		auto Buddy_block_alloc<MinSize,MaxSize>::_split(std::uint32_t layer,
		...
		template<std::uint32_t MinSize, std::uint32_t MaxSize>
		auto Buddy_block_alloc<MinSize,MaxSize>::_merge(std::uint32_t layer,

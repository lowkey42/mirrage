...
namespace graphic {
	...
	namespace {
		...
		auto locate_matching_pipeline_cache(vk::PhysicalDevice gpu, asset::Asset_manager& assets) {
			...
			if(caches.size()>=max_pipeline_cache_count) {
				DEBUG("More than "<<max_pipeline_cache_count<<" pipeline cahces. Deleting oldest caches.");

				std::sort(std::begin(caches), std::end(caches), [&](auto& lhs, auto& rhs) {
					return assets.last_modified(lhs).get_or_other(-1) > assets.last_modified(rhs).get_or_other(-1);
				});
		...
		auto load_pipeline_cache(const vk::Device& device, asset::Asset_manager& assets, const asset::AID& id) {
		...
		void store_pipeline_cache(const vk::Device& device, asset::Asset_manager& assets,
		                          const asset::AID& id, vk::PipelineCache cache) {
	...
	Swapchain::Swapchain(const vk::Device& dev, Window& window, vk::SwapchainCreateInfoKHR info)
	    ...
	    , _image_width(info.imageExtent.width), _image_height(info.imageExtent.height)
	...
	auto Swapchain::acquireNextImage(vk::Semaphore s, vk::Fence f)const -> std::size_t {
		return _device.acquireNextImageKHR(*_swapchain, std::numeric_limits<std::uint64_t>::max(),
		                                   s,f).value;
	}
	...
	Device::Device(Context& context, asset::Asset_manager& assets,
	               vk::UniqueDevice device, vk::PhysicalDevice gpu,
	               Queue_tag transfer_queue, Queue_family_mapping queue_mapping,
	               Swapchain_create_infos swapchains, bool dedicated_alloc_supported)
	    ...
	    , _depth_format(get_supported_format(
	          {vk::Format::eD24UnormS8Uint, vk::Format::eD32SfloatS8Uint,
	           vk::Format::eD32Sfloat, vk::Format::eD16Unorm, vk::Format::eD16UnormS8Uint},
	          vk::FormatFeatureFlagBits::eDepthStencilAttachment,
	          Format_usage::image_optimal).get_or_throw("No depth format supported"
	                                                    " by device"))
		, _depth_stencil_format(get_supported_format(
	          {vk::Format::eD24UnormS8Uint, vk::Format::eD32SfloatS8Uint,
	           vk::Format::eD16UnormS8Uint},
	          vk::FormatFeatureFlagBits::eDepthStencilAttachment,
	          Format_usage::image_optimal).get_or_throw("No depth-stencil format "
	                                                    "supported by device"))
	    ...
	    , _sr_format(get_supported_format(
	          {vk::Format::eR8Srgb},
	          vk::FormatFeatureFlagBits::eBlitDst
	          | vk::FormatFeatureFlagBits::eBlitSrc
	          | vk::FormatFeatureFlagBits::eSampledImageFilterLinear,
		...
		for(auto& sc_info : swapchains) {
			_swapchains.emplace(sc_info.first, Swapchain{*_device, *std::get<0>(sc_info.second),
			                                             std::get<1>(sc_info.second)});
		}
	}
	...
	void Device::print_memory_usage(std::ostream& log)const {
		...
		for(auto& t : mem_usage.types) {
			...
			log<<"      "<<"temporary : "
			   << t.second.temporary_pool.used<<"/"
			   << t.second.temporary_pool.reserved<<" in "
			   << t.second.temporary_pool.blocks<<" blocks\n";
			
			log<<"      "<<"normal    : "
			   << t.second.normal_pool.used<<"/"
			   << t.second.normal_pool.reserved<<" in "
			   << t.second.normal_pool.blocks<<" blocks\n";
			
			log<<"      "<<"persistent : "
			   << t.second.persistent_pool.used<<"/"
			   << t.second.persistent_pool.reserved<<" in "
			   << t.second.persistent_pool.blocks<<" blocks\n";
		}
	...
	auto Device::get_queue(Queue_tag tag) -> vk::Queue {
		...
		return _device->getQueue(std::get<0>(real_familiy->second),
		                         std::get<1>(real_familiy->second));
	}
	...
	auto Device::create_semaphore() -> vk::UniqueSemaphore {
		return _device->createSemaphoreUnique({});
	}

	auto Device::create_buffer(vk::BufferCreateInfo info, bool host_visible,
		...
		auto mem = [&]{
			...
			return _memory_allocator.alloc(mem_req.size, mem_req.alignment,
			                               mem_req.memoryTypeBits,
			                               host_visible,
			                               lifetime).get_or_throw();
		}();
	...
	auto Device::create_image(vk::ImageCreateInfo info, bool host_visible,
	                          Memory_lifetime lifetime,
		...
		auto mem = [&]{
			if(dedicated) {
				return _memory_allocator.alloc_dedicated(*image,
				                                         host_visible).get_or_throw();
			}
	...
	auto Device::create_image_view(vk::Image image, vk::Format format,
	                               std::uint32_t base_mipmap, std::uint32_t mipmap_levels,
		...
		return _device->createImageViewUnique(vk::ImageViewCreateInfo{
				vk::ImageViewCreateFlags{}, image, type, format, mapping, range
		});
	}
	...
	auto Device::create_sampler(std::uint32_t mip_levels,
		...
		return _device->createSamplerUnique(vk::SamplerCreateInfo{
		                                        ...
		                                        filter, filter, mipmap_mode,
		                                        address_mode, address_mode, address_mode,
		                                        0.f, anisotropic, max_aniso,
		                                        depth_compare_op!=vk::CompareOp::eAlways,
		                                        depth_compare_op,
		                                        0.f, float(mip_levels), border_color,
		                                    });
	}


	auto Device::create_command_buffer_pool(Queue_tag queue_family, bool resetable,
	                                        bool short_lived) -> Command_buffer_pool {
		auto real_familiy = _queue_family_mappings.find(queue_family);
		INVARIANT(real_familiy!=_queue_family_mappings.end(), "Unknown queue family tag: "<<queue_family.str());
		...
		return {*_device, _device->createCommandPoolUnique(vk::CommandPoolCreateInfo{
			flags, std::get<0>(real_familiy->second)
		})};
	}

	auto Device::create_descriptor_set_layout(gsl::span<const vk::DescriptorSetLayoutBinding> bindings
	                                          ) -> vk::UniqueDescriptorSetLayout {
		return _device->createDescriptorSetLayoutUnique(vk::DescriptorSetLayoutCreateInfo{
				vk::DescriptorSetLayoutCreateFlags{}, gsl::narrow<std::uint32_t>(bindings.size()), bindings.data()
		});
	}
	...
	auto Device::create_descriptor_pool(std::uint32_t maxSets,
	                                    std::vector<vk::DescriptorPoolSize> pool_sizes
	                                   ) -> Descriptor_pool {
		return {*_device, _device->createDescriptorPoolUnique(vk::DescriptorPoolCreateInfo{
			vk::DescriptorPoolCreateFlagBits::eFreeDescriptorSet,
			maxSets, gsl::narrow<std::uint32_t>(pool_sizes.size()), pool_sizes.data()
		})};
	}

	auto Device::create_fence() -> Fence {
		return Fence{*_device};
	}
	...
	auto Device::get_supported_format(std::initializer_list<vk::Format> formats,
		...
		for(auto format : formats) {
			...
			auto features  = [&] {
				switch(usage) {
					case Format_usage::buffer:
						return props.bufferFeatures;
					case Format_usage::image_linear:

...
namespace graphic {
	...
	auto Descriptor_pool::create_descriptor(vk::DescriptorSetLayout layout) -> vk::UniqueDescriptorSet {
		return std::move(_device.allocateDescriptorSetsUnique(vk::DescriptorSetAllocateInfo{
			*_pool, 1, &layout
		})[0]);
	}
	...
	void Image_descriptor_set_layout::update_set(vk::DescriptorSet set,
		...
		for(auto& desc_image : desc_images) {
			desc_writes.emplace_back(set, desc_writes.size(), 0, 1,
	...
	auto create_fence(Device& d) -> Fence {
		return d.create_fence();
	}
	
	
	
	Framebuffer::Framebuffer(vk::UniqueFramebuffer fb, vk::Viewport viewport,
	                         vk::Rect2D scissor, std::vector<vk::ClearValue> cv)
	    : _fb(std::move(fb)), _viewport(viewport), _scissor(scissor)
	    , _clear_values(std::move(cv)) {
	}
	...
	void image_layout_transition(vk::CommandBuffer cb,
		...
		auto subresource = vk::ImageSubresourceRange {
			aspects, mip_level, mip_level_count, 0, 1
		};

		auto barrier = vk::ImageMemoryBarrier {
			get_access_mask(src_layout), get_access_mask(dst_layout),
			src_layout, dst_layout, VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED,
			image, subresource
		};
	...
	void generate_mipmaps(vk::CommandBuffer cb,
		...
		if(src_layout!=vk::ImageLayout::eTransferSrcOptimal) {
			image_layout_transition(cb, image, src_layout,
			                        vk::ImageLayout::eTransferSrcOptimal,
			                        vk::ImageAspectFlagBits::eColor, start_mip_level, 1);
		}

		for(auto level : util::range(1+start_mip_level, mip_count-1)) {
			image_layout_transition(cb, image, vk::ImageLayout::eUndefined,
			                        vk::ImageLayout::eTransferDstOptimal,
			                        vk::ImageAspectFlagBits::eColor, level, 1);

			auto src_range = std::array<vk::Offset3D, 2> {
				vk::Offset3D{0,0,0},
				vk::Offset3D{
					int32_t(width >> (level - 1)),
					int32_t(height >> (level - 1)),
					1}
			};
		...
		if(dst_layout!=vk::ImageLayout::eTransferSrcOptimal) {
			image_layout_transition(cb, image,
			                        vk::ImageLayout::eTransferSrcOptimal,
			                        dst_layout,
			                        vk::ImageAspectFlagBits::eColor,
			                        start_mip_level, mip_count-start_mip_level);
		}
	}
	...
	void blit_texture(vk::CommandBuffer cb,
		...
		if(initial_src_layout!=vk::ImageLayout::eTransferSrcOptimal) {
			image_layout_transition(cb, src.image(), initial_src_layout,
			                        vk::ImageLayout::eTransferSrcOptimal,
			                        vk::ImageAspectFlagBits::eColor, 0, 1);
		}
		
		if(initial_dst_layout!=vk::ImageLayout::eTransferDstOptimal) {
			image_layout_transition(cb, dst.image(), initial_dst_layout,
			                        vk::ImageLayout::eTransferDstOptimal,
			                        vk::ImageAspectFlagBits::eColor, 0, 1);
		}
		...
		cb.blitImage(src.image(), vk::ImageLayout::eTransferSrcOptimal,
		             dst.image(), vk::ImageLayout::eTransferDstOptimal,
		             {blit}, vk::Filter::eLinear);
		
		
		if(final_src_layout!=vk::ImageLayout::eTransferSrcOptimal) {
			image_layout_transition(cb, src.image(), vk::ImageLayout::eTransferSrcOptimal,
			                        final_src_layout,
			                        vk::ImageAspectFlagBits::eColor, 0, 1);
		}
		
		if(final_dst_layout!=vk::ImageLayout::eTransferDstOptimal) {
			image_layout_transition(cb, dst.image(), vk::ImageLayout::eTransferDstOptimal,
			                        final_dst_layout,
			                        vk::ImageAspectFlagBits::eColor, 0, 1);
		}
	}

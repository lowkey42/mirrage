...
namespace graphic {
	...
	Profiler::Push_raii::Push_raii(Push_raii&& rhs)noexcept
	...
	void Profiler::start(vk::CommandBuffer cb) {
		...
		cb.writeTimestamp(vk::PipelineStageFlagBits::eTopOfPipe, *_query_pools.head(),
	...
	void Profiler::end  () {
		...
		auto& cb = _current_command_buffer.get_or_throw("No active command buffer! Has Profiler::start been called?");

		cb.writeTimestamp(vk::PipelineStageFlagBits::eBottomOfPipe, *_query_pools.head(),
		...
		_query_pools.pop_while([&](auto& pool) {
			// receivce results
			auto status = _device.getQueryPoolResults<std::uint32_t>(
			                  *pool, 0, _next_query_id, _query_result_buffer,
			                  sizeof(std::uint32_t), vk::QueryResultFlags{});
	...
	auto Profiler::push(const std::string& name, vk::PipelineStageFlagBits stage) -> Push_raii {
		...
		cb.writeTimestamp(stage, *_query_pools.head(),
		                  result.query_id_begin());
	...
	auto Profiler::_generate_query_id() -> std::uint32_t {
		INVARIANT(_next_query_id < _query_ids, "More Profiler::push calls than supported!"
		          "Requested " << (_next_query_id+1) << ", supported " << _query_ids);

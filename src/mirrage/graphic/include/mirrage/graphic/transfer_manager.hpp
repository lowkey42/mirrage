...
namespace mirrage::graphic {
	...
	class Static_buffer {
	  public:
		...
		auto transfer_task() const { return _transfer_task; }
	...
	class Dynamic_buffer {
	  public:
		Dynamic_buffer(Backed_buffer          buffer,
		               ...
		               vk::AccessFlags        latest_usage_access)
	...
	class Transfer_manager {
	  public:
		...
		/**
			 ...
			 *
			 */
		auto upload_image(vk::ImageType,
		                  std::uint32_t owner,
	  ...
	  private:
		...
		struct Transfer_image_req {
			...
			Transfer_image_req(Backed_buffer              src,
			                   vk::Image                  dst,
			                   vk::DeviceSize             size,
			                   std::uint32_t              owner,
			                   std::uint32_t              mip_count_actual,
			  ...
			  , size(size)
			  , owner(owner)
			  , mip_count_actual(mip_count_actual)
		...
		async::event_task<void>  _tranfer_done_event;
		async::shared_task<void> _transfer_done_task;

		void _reset_transfer_event();

	...
	*/
	...
	struct Pipeline_stage {
		...
		void add_constant(std::uint32_t id, const T& data) {
			add_constant(id, gsl::span<const char>(reinterpret_cast<const char*>(&data),
			                                       sizeof(T)));
		}
	...
	struct Pipeline_description {
		public:
			...
			void vertex(int binding, bool per_instance_data, Member&&... members) {
				if(binding<0) binding = vertex_bindings.size();

				vertex_bindings.emplace_back(binding, sizeof(T),
				                             per_instance_data ? vk::VertexInputRate::eInstance
				                                               : vk::VertexInputRate::eVertex);

				util::apply2([&](auto location, auto& member) {
					add_vertex_attributes<decltype(util::get_member_type(member))>(binding,
					                                                               location,
					                                                               util::get_member_offset(member));
				}, members...);
			}

			void add_push_constant(util::Str_id id, std::uint32_t offset, std::uint32_t size, vk::ShaderStageFlags);
		...
		private:
			...
			void build_create_info(const vk::Device&,
	...
	class Stage_builder {
		public:
			...
			auto add_push_constant(util::Str_id id,
			                       std::uint32_t offset, std::uint32_t size,
			                       vk::ShaderStageFlags) -> Stage_builder&;
	...
	inline constexpr auto blend_premultiplied_alpha = Attachment_blend {
	...
	struct Framebuffer_attachment_desc {
		...
		Framebuffer_attachment_desc(vk::ImageView v, util::Rgba color)
		    : image_view(v), clear_value(vk::ClearColorValue(std::array<float,4>{
	...
	class Render_pass_builder {
		public:
			...
			auto add_dependency(util::maybe<Subpass_builder&> src, vk::PipelineStageFlags srcStageMask,
			                    vk::AccessFlags srcAccessMask,
			                    util::maybe<Subpass_builder&> dst, vk::PipelineStageFlags dstStageMask,
			...
			auto build_framebuffer(gsl::span<Framebuffer_attachment_desc> attachments,
			                       int width, int height, int layers=1) -> Framebuffer;
			
			auto build_framebuffer(Framebuffer_attachment_desc attachment,
			                       int width, int height, int layers=1) -> Framebuffer {
				return build_framebuffer(gsl::span<Framebuffer_attachment_desc>	{&attachment, 1},
				                         width, height, layers);
			}
	...
	class Render_pass {
		...
		private:
			...
			Render_pass(vk::UniqueRenderPass render_pass, std::vector<vk::UniquePipeline> pipelines,
	...
	template<> inline void Pipeline_description::add_vertex_attributes<float>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32Sfloat, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<std::int32_t>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32Sint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<std::uint32_t>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32Uint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::vec2>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32Sfloat, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::ivec2>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32Sint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::uvec2>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32Uint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::vec3>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32B32Sfloat, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::ivec3>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32B32Sint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::uvec3>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32B32Uint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::vec4>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32B32A32Sfloat, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::ivec4>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32B32A32Sint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<glm::uvec4>(int binding, int location, std::size_t offset) {
		vertex_attributes.emplace_back(location, binding, vk::Format::eR32G32B32A32Uint, offset);
	}
	template<> inline void Pipeline_description::add_vertex_attributes<std::uint8_t (&) [4]>(int binding, int location, std::size_t offset) {
